  ...
  return (
    <div className="flex flex-col mt-2 border-t-[1px] border-accent ">
      <div className="flex flex-row text-center justify-center p-2 h-10">
        <button className={'btn btn-xs btn-ghost rounded-btn ' + (showFriends && 'btn-active')} onClick={toggleMode}>
          好友列表
        </button>
      ...
      <div className="flex-grow space-y-2 p-2 border-t-[1px] border-accent overflow-y-scroll h-friend-list">{showFriends ? <FriendList /> : <FriendSkips />}</div>
    </div>
  )
}
          ...
          {index === showId && (
            <>
              <div className="flex flex-row space-x-4 mt-2 px-4 w-full">
                <button className={'btn btn-ghost rounded-btn btn-sm flex-grow ' + (friend.status !== 'room' && ' btn-disabled')} onClick={() => WS.joinRoomFromFriend(friend.username)}>
                  <span className="material-icons-outlined bg-transparent">sports_kabaddi</span>
                </button>
                <button className="btn btn-ghost rounded-btn btn-sm  flex-grow" onClick={() => WS.deleteFriend(friend.username)}>
  ...
  return (
    <>
      {userVM.friendSkipList.map((skip, index) => (
        <div key={index} className="flex items-center p-2 bg-base-200 rounded-lg  btn btn-ghost h-auto text-left flex-nowrap">
          <div className="flex flex-col flex-grow">
      ...
      {userVM.friendSkipList.length === 0 && <div className="text-sm text-center">没有新的好友请求</div>}
    </>
  )
}
